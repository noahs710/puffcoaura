...
if (SERVER_DIR / "index.html").exists():
    ROOT = SERVER_DIR
    TOOLS_DIR = ROOT / "tools"
else:
    TOOLS_DIR = SERVER_DIR
...
PUFFCO_MANUFACTURER_MAC_PREFIX = "F0:AD:4E"
PUFFCO_MANUFACTURER_MAC_PREFIXES = (PUFFCO_MANUFACTURER_MAC_PREFIX,)
...
def mood_light_payload(
    ...
    *,
    ...
    # di_frac is the dynamic inhale intensity 0-1. If unset, it falls
    # back to 1.0 when dynamic_inhale is True and 0.0 when False. This
    # is what the firmware uses to scale the chamber response during a
    # draw. Vapor presets map to: Smooth=0, Bold=0.33, Intense=0.66,
    # Extreme=1.0.
    ...
    if di_frac is None:
        di_frac_value = float(dynamic)
    else:
        di_frac_value = clamp_float(di_frac, 0, 1, 0.0)
    ...
    if preset_key == "no_animation":
        ...
        # Even on no_animation, when dynamic_inhale is on the firmware
        # still reads diFrac / speedDi0 / speedDi1 to modulate the
        # solid color during a draw. The web app sends no_animation
        # as the default mood for the Bold/Intense/Extreme vapor
        # presets, so we need to include these fields for the firmware
        # to react to the draw.
        if dynamic:
            params["speedDi0"] = speed_di0
            params["speedDi1"] = speed_di1
            params["diFrac"] = di_frac_value
    else:
        color_len = count * 5
...
# Vapor preset IDs → dynamic inhale intensity. Order matches the
# canonical progression: Smooth → Bold → Intense → Extreme.
VAPOR_INTENSITY = {
    "standard": 0.0,  # Smooth — no draw modulation
    "smooth": 0.0,
    "high": 0.33,     # Bold
    "bold": 0.33,
    "max": 0.66,      # Intense
    "intense": 0.66,
    "xl": 1.0,        # Extreme / 3D XL
    "extreme": 1.0,
}


def vapor_dynamic_inhale(vapor_id):
    """Returns (dynamic_inhale_bool, di_frac_float) for a vapor preset."""
    if vapor_id is None:
        return False, 0.0
    key = str(vapor_id).strip().lower()
    if key in VAPOR_INTENSITY:
        intensity = VAPOR_INTENSITY[key]
    elif key in {"standard", "balanced", "default", "normal", "low", "mild"}:
        intensity = 0.0
    elif key in {"high", "bold", "medium", "more", "boosted"}:
        intensity = 0.33
    elif key in {"max", "intense", "strong", "maximum", "full"}:
        intensity = 0.66
    elif key in {"xl", "extreme", "ultra", "xlarge", "3d_xl", "xl_vapor"}:
        intensity = 1.0
    else:
        intensity = 0.0
...
async def device_snapshot(*, full: bool = True) -> dict[str, Any]:
    ...
    try:
        ...
        # Extra OFFICIAL attribute lift-ups so the web UI can read hardware
        # diagnostics off the flat data object. The web app's
        # normalizeSnapshotForUi also flattens these, but copying them
        # here keeps the snapshot consistent across transports (CLI,
        # scripts, and the bridge) without depending on UI-side massaging.
        for src, dst in (
            ("boostTime", "boost_time_s"),
            ("boostTemperature", "boost_temperature_delta_c"),
            ("chargeEstimatedTimeToFull", "charge_estimated_time_to_full_s"),
            ...
            value = official_attrs.get(src)
            if value is not None and data.get(dst) is None:
                data[dst] = value
        ble_faults = official_attrs.get("bleFault")
        if isinstance(ble_faults, dict):
            if ble_faults.get("absoluteCount") is not None and data.get("ble_fault_absolute_count") is None:
                data["ble_fault_absolute_count"] = ble_faults["absoluteCount"]
            if ble_faults.get("creditCount") is not None and data.get("ble_fault_credit_count") is None:
                data["ble_fault_credit_count"] = ble_faults["creditCount"]
        # Derived fields: same honest derivations the web app runs.
        if data.get("battery_voltage_v") is not None and data.get("battery_current_a") is not None and data.get("charge_rate_w") is None:
            data["charge_rate_w"] = round(abs(data["battery_voltage_v"] * data["battery_current_a"]) * 10) / 10
        if data.get("cable_handshake_detected") is not None and data.get("cable_attached") is None:
            data["cable_attached"] = data["cable_handshake_detected"] != 0
        if data.get("date_of_birth_unix") and data["date_of_birth_unix"] > 0 and data.get("days_owned") is None:
            data["days_owned"] = max(0, int((time.time() - data["date_of_birth_unix"]) / 86400))
        if data.get("utc_time_unix") and data["utc_time_unix"] > 0 and data.get("device_utc_time") is None:
            from datetime import datetime, timezone
            data["device_utc_time"] = datetime.fromtimestamp(data["utc_time_unix"], tz=timezone.utc).strftime("%Y-%m-%d %H:%M:%S")
        if data.get("dab_total_time_s") is not None and data.get("total_dabs") and data["total_dabs"] > 0 and data.get("avg_seconds_per_dab") is None:
            data["avg_seconds_per_dab"] = round(data["dab_total_time_s"] / data["total_dabs"])
        if data.get("charge_source") is not None and data.get("charge_source_label") is None:
            labels = {0: "On battery", 1: "USB", 2: "Wireless"}
            data["charge_source_label"] = labels.get(data["charge_source"], f"Source {data['charge_source']}")
        ...
        if full or not data.get("profiles"):
            ...
            for index in range(PROFILE_COUNT):
                ...
                # Decode the dynamic inhale flag (and diFrac if present)
                # back into a vapor preset ID. The UI displays
                # Smooth / Bold / Intense / Extreme from this field.
                color = profile.get("color") or {}
                meta = color.get("meta") or {}
                param = (color.get("lamp") or {}).get("param") or {}
                di_frac = param.get("diFrac", meta.get("diFrac"))
                dynamic_on = bool(meta.get("dynamicInhale", param.get("diFrac", 0)))
                if dynamic_on and isinstance(di_frac, (int, float)):
                    intensity = float(di_frac)
                    if intensity <= 0.16:
                        profile["vapor"] = "standard"
                    elif intensity <= 0.50:
                        profile["vapor"] = "high"
                    elif intensity <= 0.83:
                        profile["vapor"] = "max"
                    ...
                    profile["vapor"] = "standard"
                profile["vapor_intensity"] = float(di_frac) if isinstance(di_frac, (int, float)) else 0.0
                profile["vapor_dynamic_inhale"] = dynamic_on
                profile["labels"] = {
...
def normalize_mac_prefix(value: Any) -> str:
    raw = str(value or "").strip().upper()
    compact = "".join(ch for ch in raw if ch in "0123456789ABCDEF")
    if len(compact) >= 6:
        return ":".join(compact[i : i + 2] for i in range(0, 6, 2))
    parts = [part for part in normalize_mac_address(value).split(":") if part]
    return ":".join(parts[:3]) if len(parts) >= 3 else ""


def scan_mac_prefixes(params: dict[str, Any]) -> tuple[str, ...]:
    explicit = params.get("manufacturer_prefixes") or params.get("mac_prefixes")
    if not explicit and params.get("manufacturer_prefix"):
        explicit = [params["manufacturer_prefix"]]
    if isinstance(explicit, str):
        explicit = [explicit]
    prefixes = tuple(
        dict.fromkeys(
            prefix
            for prefix in (normalize_mac_prefix(item) for item in explicit or PUFFCO_MANUFACTURER_MAC_PREFIXES)
            if prefix
        )
    )
    return prefixes or PUFFCO_MANUFACTURER_MAC_PREFIXES


def matches_mac_prefix(address: Any, prefixes: tuple[str, ...]) -> bool:
...
async def handle_scan_devices_locked(params: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "data": {
            ...
            "backend": "windows_python_ble",
            "timeout_s": timeout,
            "manufacturer_prefix": prefixes[0],
            "manufacturer_prefixes": list(prefixes),
            "note": "" if rows else f"No Puffco devices found with MAC prefix {prefix_label}.",
        },
    }
...
async def write_profile_fields(index: int, params: dict[str, Any]) -> tuple[list[str], bytes | None]:
    ...
    # Resolve the vapor setting first — if the caller sent vapor
    # without an explicit mood_light, we synthesize a no_animation
    # mood light payload with the matching dynamic inhale intensity.
    vapor_id = params.get("vapor")
    vapor_di_on, vapor_di_frac = vapor_dynamic_inhale(vapor_id)

    if isinstance(params.get("mood_light"), dict):
        mood = params["mood_light"]
        # Vapor is the source of truth for dynamic inhale. If the
        # caller asked for Bold/Intense/Extreme, the dynamic_inhale
        # flag MUST be true. We OR with whatever the mood editor set
        # so a pulsing-mood profile with Smooth vapor keeps its pulse.
        mood_di_inhale = mood.get("dynamic_inhale", mood.get("dynamicInhale", False))
        if vapor_di_on or mood_di_inhale:
            effective_di = True
        else:
            effective_di = False
        # If the caller provided an explicit di_frac, use it.
        # Otherwise, fall back to the vapor's intensity when vapor is
        # on, or 1.0 when the mood editor turned it on but vapor is
        # off.
        explicit_di_frac = mood.get("di_frac", mood.get("diFrac"))
        if explicit_di_frac is not None:
            effective_di_frac = clamp_float(explicit_di_frac, 0, 1, 1.0)
        elif vapor_di_on:
            effective_di_frac = vapor_di_frac
        elif mood_di_inhale:
            effective_di_frac = 1.0
        else:
            effective_di_frac = 0.0
        expected_mood_payload = mood_light_payload(
            ...
            dynamic_inhale=effective_di,
            di_frac=effective_di_frac,
        )
        ...
        # dynamic inhale. Send a no_animation mood light with the
        # matching diFrac so the firmware's profile-colour write stores
        # the dynamicInhale flag.
        expected_mood_payload = mood_light_payload(
            "no_animation",
            ["#ffffff"],
            tempo_frac=0.5,
            dynamic_inhale=True,
            di_frac=vapor_di_frac,
        )
        await device.set_profile_colour(
            index,
            colour=expected_mood_payload,
        )
        changed.append(f"vapor:{vapor_id}")
