...
class PuffcoBLE:
    ...
    async def trigger_bonding(self) -> bool:
        ...
        # Retry reading the characteristic a few times. 
        # Bypassing the cache can sometimes mean Windows needs a moment to finish service discovery.
        for attempt in range(3):
            try:
                await self.client.read_gatt_char(LoraxService.VERSION_CHAR)
                return True
            except Exception as e:
                self.debug_print(f"trigger_bonding read failed (attempt {attempt+1}): {e}")
                await asyncio.sleep(1.0)
                
        # Final attempt that will raise if it fails
        await self.client.read_gatt_char(LoraxService.VERSION_CHAR)
        return True
    ...
    async def get_battery_level(self) -> int:
        val = float(await self.read("/p/bat/soc", 0, 4, "float32"))
    ...
    async def is_lantern_mode(self) -> bool:
        lantern = await self.read("/p/app/ltrn/cmd", 0, 1, data_type='uint8')
        return int(lantern) == 1
