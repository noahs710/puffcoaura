class PuffcoBrowserBleClient {
  ...
  async connect(params = {}) {
    ...
    // The chooser filter has to be loose because Puffco hardware has
    // shipped under multiple Bluetooth identifiers and at least one
    // user-reported case where the device's advertised name is
    // something custom (e.g. "Pretty Peaky" / "Pupizzle") that doesn't
    // start with the Peak/Puffco/Proxy prefixes the official app
    // targets, AND the Lorax service UUID isn't included in the
    // advertisement packet (Bleak sees `svc=[]` on Puffco scans) — so
    // the strict { services / namePrefix } filter would silently hide
    // every Puffco in range. acceptAllDevices + optionalServices lets
    // the user pick their Puffco from the full chooser; the connection
    // step still validates the Lorax service before committing.
